//
// Created by W on 2019/2/25 0025.
//
// 面试题26：树的子结构
...
public:
    ...
    bool HasSubtree(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2)
    {
        if(!pRoot1 || !pRoot2)
            return false;
        return dfs(pRoot1, pRoot2) || HasSubtree(pRoot1->m_pLeft, pRoot2)
               || HasSubtree(pRoot1->m_pRight, pRoot2);
    }

    bool dfs(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2)
    {
        if(!pRoot2)
            return true;
        if(!pRoot1)
        ...
        if(pRoot1->m_nValue != pRoot2->m_nValue)
            return false;
        return dfs(pRoot1->m_pLeft, pRoot2->m_pLeft) &&
               dfs(pRoot1->m_pRight, pRoot2->m_pRight);
    }
