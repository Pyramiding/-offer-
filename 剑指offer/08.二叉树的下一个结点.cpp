...
public:
    BinaryTreeNode* GetNext(BinaryTreeNode* pNode)
    {
        ...
        {
            BinaryTreeNode *pRoot = pNode->m_pParent;
            if(pRoot->m_pLeft == pNode)
                return pRoot;
...
BinaryTreeNode* GetNext(BinaryTreeNode* pNode)
{
    ...
    if(pNode->m_pRight != nullptr) //寻找该节点右子树的最左节点
    {
...
// 二叉树的前一个结点
BinaryTreeNode* GetBefore(BinaryTreeNode* pNode)
{
    if(pNode == nullptr)
        return nullptr;

    BinaryTreeNode* pBefore = nullptr;
    if(pNode->m_pLeft != nullptr) //寻找该节点左子树的最右节点
    {
    ...
    else if(pNode->m_pParent != nullptr)
    {
        BinaryTreeNode* pCurrent = pNode;
        BinaryTreeNode* pParent = pNode->m_pParent;
        while(pParent != nullptr && pCurrent == pParent->m_pLeft)
        {   // 向上遍历，直到找到某节点为其父节点的右子节点
            pCurrent = pParent;
            pParent = pParent->m_pParent;
        }
